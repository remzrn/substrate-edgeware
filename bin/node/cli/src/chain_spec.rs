...
pub use node_runtime::constants::{time::*};
...
fn get_lockdrop_participants_allocation() -> serde_json::Result<Allocation>{
	let path = std::path::Path::new("bin/node/cli/lockdrop/lockdrop-allocation.json");
	let mut file = std::fs::File::open(&path).unwrap();
	let mut data = String::new();
	file.read_to_string(&mut data).unwrap();
	let a: Allocation = jsonrpc_core::serde_json::from_str(&data)?;
	return Ok(a);
}
	...
	GenesisConfig {
		...
		pallet_balances: Some(BalancesConfig {
			balances: endowed_accounts.iter().cloned().map(|k| (k, STASH))
				.chain(founder_allocation.clone())
...
fn edgeware_testnet_config_genesis() -> GenesisConfig {
	...
	testnet_genesis(
		crate::testnet_fixtures::get_mtestnet_initial_authorities(),
...
pub fn edgeware_testnet_config(testnet_name: String, testnet_node_name: String) -> ChainSpec {
	...
	let properties = jsonrpc_core::serde_json::from_str(data).unwrap();
...
pub fn mainnet_genesis(
	...
	GenesisConfig {
		...
		pallet_session: Some(SessionConfig {
			keys: initial_authorities.iter().map(|x| {
				(x.0.clone(), x.1.clone(), session_keys(x.4.clone(), x.3.clone(), x.5.clone(), x.6.clone()))
			}).collect::<Vec<_>>(),
		}),
		pallet_staking: Some(StakingConfig {
			validator_count: 60,
		...
		pallet_democracy: Some(DemocracyConfig::default()),
		// pallet_elections_phragmen: Some(ElectionsConfig {
		// 	members: balances.iter()
		// 				.take((balances.len() + 1) / 2)
		// 				.cloned()
		// 				.map(|member| (member, STASH))
		// 				.collect(),
		// }),
